...
weekly_cases <- read.csv("weekly_demo.csv")

## Specify the health zones we are plotting, change date to a week
weekly_cases2  <- weekly_cases %>%
  group_by(SCOnset) %>%
  mutate(cum_confirm = cumsum(weekly_confirm)) %>%
  filter(SCOnset %in% c("beni", "butembo", 'kalunguta', 'katwa', 'mabalako', 'mandima')) %>%
...
week_number <- weekly_cases2 %>%
  arrange(Date)
weeks <- as.data.frame(unique(week_number$Date))
...
ggplot(weekly_cases2, aes(fill=SCOnset, x=Week, y=weekly_confirm)) + labs(fill = "Health Zone") +
